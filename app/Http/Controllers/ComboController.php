...
class ComboController extends Controller
{
    ...
    public function products (Product $combo){
        $productIds= $combo->sub->pluck('product_id')->toArray();
        $products=Product::whereNull('type')->whereNotIn('id', $productIds)->latest()->get();
        return view('combo.products',[
            'combo'=>$combo->load('sub.product'),
            'products'=>$products,
        ]);
    }

    public function productDelete(SubProduct $subProduct){
        $subProduct->delete();
        return redirect()->back()->with('success',"Combo item removed.");
    }
