...
@section('content')
    ...
    <div class="px-3">
        <div class="d-flex justify-content-between align-items-center notPrint">
            @include('report.menu')
            <div class="d-flex">
                <a href="{{ route('report.monthly') }}?type=pdf" class="btn text-dark"><i class="fa fa-file mr-2"></i>PDF</a>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <div class="printOnly">
                    ...
                    <i class="d-block text-center mb-3">(Monthly Report)</i>
                </div>
                <div class="d-flex">
                    <x-per-page></x-per-page>
                </div>
                <x-table-component :headers="['S.N', 'Product', 'Unit Price','Received Amount','Returned Amount','S.N. number', 'Date']">
                    @foreach ($reports as $report)
                        <tr class="text-center">
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $report->product->name }}</td>
                            <td>Rs. {{ $report->price }}</td>
                            <td>Rs. {{ $report->received_amount ?? 0 }}</td>
                            <td>Rs. {{ $report->returned_amount ?? 0 }}</td>
